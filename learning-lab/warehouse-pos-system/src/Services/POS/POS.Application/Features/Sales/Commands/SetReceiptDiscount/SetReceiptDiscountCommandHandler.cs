...
{
    public class SetReceiptDiscountCommandHandler : IRequestHandler<SetReceiptDiscountCommand, SaleDto>
    {
        ...
        public SetReceiptDiscountCommandHandler(
        ...
        public async Task<SaleDto> Handle(SetReceiptDiscountCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                throw new ConflictException($"Sale {sale.Id} is {sale.Status}; the receipt discount can only be changed on a sale that is still InProgress.");
            }

            sale.ManualReceiptDiscountPercent = request.ManualReceiptDiscountPercent;
